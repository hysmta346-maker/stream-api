...
// استخدام خدمة AllOrigins وهي أكثر موثوقية
const getViaProxy = async (url) => {
    const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(url)}`;
    const response = await axios.get(proxyUrl, {
        headers: { 'User-Agent': USER_AGENT }
    });
    return response.data.contents; // الخدمة تعيد HTML داخل مفتاح contents
};
...
app.get('/api/stream/movie', async (req, res) => {
    ...
    try {
        ...
        if (match) {
            ...
            return res.status(404).json({ success: false, message: "تعذر استخراج رابط البث" });
        }
...
app.get('/', (req, res) => {
    res.status(200).send('StreamMaster Ultimate Proxy API is running! 🚀');
});
